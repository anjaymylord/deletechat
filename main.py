        ...
        if "https://t.me/c/" in message.text:
            ...
            if acc is None:
                context.bot.send_message(
                    message.chat.id,
                    f"**String Session is not Set**",
                    reply_to_message_id=message.id
                )
                return
            try:
                handle_private(message, chatid, msgid, context)
            except Exception as e:
                context.bot.send_message(
                    message.chat.id,
                    f"**Error** : __{e}__",
        ...
        # public
            ...
            try:
                ...
                if acc is None:
                    ...
                    return
                try:
                    handle_private(message, username, msgid, context)
                except Exception as e:
                    context.bot.send_message(
                        message.chat.id,
                        f"**Error** : __{e}__",
...
# Fungsi untuk mendapatkan riwayat pesan
def get_chat_history(update, context):
    chat_id = update.message.chat_id
    messages = context.bot.get_chat_history(chat_id=chat_id, limit=10)

    for message in messages:
        # Lakukan pemrosesan sesuai kebutuhan Anda
        # Contoh: Cetak teks pesan
        print(message.text)

# Daftarkan handler untuk command /gethistory
get_history_handler = CommandHandler('gethistory', get_chat_history)
dispatcher.add_handler(get_history_handler)

# Daftarkan handler untuk menangani pesan dari tautan
